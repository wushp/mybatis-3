...
public class HeavyInitialLoadTest {
     ...
     * ognl on mybatis.
     * 
     * Use the thing mapper for the first time in multiple threads. The mapper contains a lot of
     * ognl references to static final class members like:
     * 
     * <code>@org.apache.ibatis.submitted.heavy_initial_load.Code@_1.equals(code)</code>
     * 
     * Handling of these references is optimized in ognl (because they never change), but version
     * 2.6.9 has a bug in caching the result . As a result the reference is translated to a 'null'
     * value, which is used to invoke the 'equals' method on (hence the 'target is null for method
     * equals' exception).
     */
    @Test

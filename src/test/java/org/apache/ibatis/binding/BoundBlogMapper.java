...
public interface BoundBlogMapper {
    ...
    @TypeDiscriminator(column = "draft", javaType = int.class, cases = {
            @Case(value = "1", type = DraftPost.class, results = {@Result(id = true, property = "id", column = "id")})})
    ...
    @ConstructorArgs({@Arg(column = "id", javaType = int.class, id = true),
            ...
            @Arg(column = "author_id", javaType = Author.class,
    ...
    @Results({
            ...
            @Result(property = "posts", column = "id",
                    many = @Many(select = "selectPostsById", fetchType = FetchType.EAGER))})

...
/**
 * This exception is thrown if a <code>java.sql.BatchUpdateException</code> is caught during the
 * execution of any nested batch. The exception contains the java.sql.BatchUpdateException that is
 * the root cause, as well as the results from any prior nested batch that executed successfully.
 * 
 * @author Jeff Butler
 */
public class BatchExecutorException extends ExecutorException {
    ...
    /*
     * Returns the BatchUpdateException that caused the nested executor to fail. That exception contains
     * an array of row counts that can be used to determine exactly which statemtn of the executor
     * caused the failure (or failures).
     *
    ...
    /*
     * Returns a list of BatchResult objects. There will be one entry in the list for each successful
     * sub-executor executed before the failing executor.
     *
     * @return the previously successful executor results (may be an empty list if no executor has
